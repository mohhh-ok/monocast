...
- 1記事あたり 3〜5 文、かつ 120〜200 字。見出しだけで終わらせず、概要から少し踏み込んで背景や要点も触れる。文を短く切り詰めて 120 字を下回るのは不可。
...
- 段落の区切りには必ず改行 \\n を入れる。1段落は最大でも 200 字以内に収め、改行のない長文ベタ書きは禁止。1記事につき 1 段落以上。
- 各記事の文数（3〜5文）・字数（120〜200字）と全件紹介のルールを厳守すること。記事を端折って短く済ませる原稿は不可。`;
...
body は必ず段落ごとに改行 \\n で区切り、1記事の段落は 120〜200 字に収めてください。改行ゼロのベタ書きや、120 字に満たない短い段落は不可です。

出力例（形式の参考。トピックは真似しない）:
{"title":"夜のききながしニュース","body":"こんばんは。ききながしラジオの時間です。今夜もいくつかの話題を順番にお届けします。\\n最初の話題です。海外の研究機関が、これまでより小型でありながら同等の性能を持つ人工知能モデルを公開しました。省電力で動くため、手元のパソコンでも扱える可能性があるそうです。\\n続いてのニュースです。国内では、地域の図書館が深夜まで開館する取り組みを始めました。仕事帰りの利用者から、静かに過ごせる場所が増えてうれしいという声が出ています。\\n次の話題です。鉄道会社が、混雑を避けて利用できる時間帯ごとの割引運賃を試験導入しました。通勤時間をずらす後押しになればという狙いです。\\nそれでは今日はこのあたりで。よい夜をお過ごしください。"}`;
