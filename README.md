...
ニュースを音声にして流しっぱなしにし、作業しながら・移動しながら情報収集するためのツール。ローカル LLM もしくはクラウド LLM で台本を書き、お好みの音声合成エンジンで読み上げる、ローカル完結・自分専用。音楽は流さず、情報のトーク主体。私的使用の範囲で動かす前提（公衆送信はしない）。
...
ブラウザを開かず、ターミナルだけで再生し続けることもできる（macOS 専用・`afplay` を使う）。

```bash
pnpm play
```

ブラウザ版と同じく、キューが 2 本未満になると裏で番組を補充しながら、完成した番組をセグメント順に再生する。`n` で番組スキップ、`q` / Ctrl+C で終了。プロファイルや各種設定はブラウザ版と同じ `data/` を共有するので、設定の変更は従来どおりトップ画面（`pnpm dev`）から行う。起動時に、合成途中で落ちて残った未完成番組は破棄される。生成と再生は同一プロセスで並行に回るので、番組間はほぼ途切れない。
